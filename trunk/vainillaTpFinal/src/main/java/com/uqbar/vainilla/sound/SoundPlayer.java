...
public class SoundPlayer {
	...
	private byte[] readBytes(int bytesToWrite) {
		...
		boolean replayFirstSong = false;
		SoundPlay firstSong = null;
		synchronized(this) {
			...
			while(iterator.hasNext()) {
				if(iterator.next().writeSamples(samplesToWrite, buffer)) {
					if (i == 0 && this.hasMusic()){
						replayFirstSong = true;
						try {
							firstSong = new SoundPlay(this.buffers.get(0).getSound(), this.buffers.get(0).getVolume());
		...
		if (replayFirstSong){
			this.buffers.add(0, firstSong);
		}

		return answer;
	}
	
	protected void addSoundAtStart(Sound sound, float volume){
		if (this.hasMusic()){
		...
		try {
			this.buffers.add(0, new SoundPlay(sound, volume));
		} catch (Exception e) {e.printStackTrace();}
	}

	public List<SoundPlay> getBuffers() {
		return buffers;
	}
